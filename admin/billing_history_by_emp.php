...
// If filtered by employee
if (isset($_GET['user_id']) && !empty($_GET['user_id'])) {
    $user_id = $_GET['user_id'];
    $where = "WHERE bm.employee_id=$user_id";
}
...
<body>
    ...
    <div class="container bg-white p-4 rounded shadow">
        ...
        <form method="get" class="row g-3 mb-4">
            <div class="col-md-4">
                <strong><label for="from" class="form-label">Select Employee</label></strong>
                <select name="user_id" id="user_id" class="form-control">
                    <option value="">-- Select --</option>
                    <?php
                        $sql1=mysqli_query($con, "select u_id, u_name from users where role='employee'");
                        while($sql=mysqli_fetch_assoc($sql1)){
                    ?>
                    <option value="<?= $sql['u_id'] ?>" <?= (isset($_GET['user_id']) && $_GET['user_id'] == $sql['u_id']) ? 'selected' : '' ?>><?= $sql['u_name'] ?></option>
                    <?php } ?>
